		...
		<section class="ftco-section ftco-cart">
						    ...
						    <tbody>
								...
								<tr class="text-center">
								  ...
								  {{ Form::open(array(
									...
									?>
										<td class="poppins">
											<p>{{number_format((float)$totalPriceProduct, 2,'.','')}} €</p>
    		...
    		<div class="row justify-content-end">
    			<div class="col-lg-8 mt-5 cart-wrap ftco-animate shadow-lg p-3 mb-5 bg-white rounded">
						...
						<div class="container">
							<div class="row">
								@foreach($produitsAccompagnements as $produitsAccompagnement)
									<div class="col-md-6 col-lg-3 ftco-animate">
										<div class="product mb-0">
											<a href="{{URL::to('/single_product/'.$produitsAccompagnement->id)}}" class="img-prod"><img class="img-fluid" src="{{$publicImageStorage}}/cover_images/{{$produitsAccompagnement->product_image}}" alt="{{$produitsAccompagnement->product_image}}">
												<div class="overlay"></div>
											</a>
											<div class="text pt-3 pb-0 px-3 text-center">
												<h3><a href="{{URL::to('/ajouter_au_panierPagePanier/'.$produitsAccompagnement->id)}}">{{$produitsAccompagnement->nom_produit}}</a></h3>
												<div class="d-flex">
													<div class="pricing">
														<p class="price"><span class="price-sale">{{$produitsAccompagnement->prix}}</span></p>
													</div>
												</div>
												<div class="bottom-area d-flex px-3">
													<div class="m-auto d-flex">
														<a href="{{URL::to('/single_product/'.$produitsAccompagnement->id)}}" class="add-to-cart d-flex justify-content-center align-items-center text-center">
															<span><i class="ion-ios-menu"></i></span>
														</a>
														<a href="{{URL::to('/ajouter_au_panierPagePanier/'.$produitsAccompagnement->id)}}" class="buy-now d-flex justify-content-center align-items-center mx-1">
    			...
    			<div class="col-lg-4 mt-5 cart-wrap ftco-animate" style="shadow p-3 mb-5 bg-white rounded">
    				<div class="cart-total mb-3">
    					...
    					<p class="d-flex">
    						<span>Dont TVA</span>
    						<span>{{number_format((float)$tva, 2,'.','')}} €</span>
    					</p>
    					<hr>
    					<p class="d-flex total-price">
    						<span>Total de votre commande</span>
							<span>{{number_format((float)$totalPrice, 2,'.','')}} €</span>
						...
						@if ($totalPrice < 20)
							...
							<p class="alert alert-info"> Pour {{number_format((float)$ecartLivraison, 2,'.','')}} euros supplémentaires, vous pouvez bénéficiez de la livraison gratuitement!</p>
						@endif

    				</div>
    				<p><a href="{{URL::to('/paiement')}}" class="btn btn-secondary py-3 px-4">Passer au paiement</a></p>
    			</div>
    		</div>
			</div>
		</section>
